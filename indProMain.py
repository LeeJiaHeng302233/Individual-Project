...
def get_tax_relief():
    ...
    while True:
        spouse = input("\nDo you have a spouse? (y/n) :D : ").lower()       #ask for y/n and return it in lowercase always 
        if spouse == 'y':
            while True:
                try:
                    ...
                    if spouse_income <= 4000:
                        ...
                        print("No spouse relief if income is above RM4000.")
                        break                                               #if spouse has income above 4000, exit the loop
                except ValueError:
                    print("Please enter a valid amount")
            break                                                           #exit the spouse loop after processing the income
        elif spouse == 'n':
            break                                                           #exit the spouse loop if the user selects 'n'
        else:
            print("Invalid input! Please enter only 'y' or 'n'.")           # prompt again if input is invalid
    ...
    while True:
        ...
        if user_choice == "1":
            ...
            if current_user is None:
                print("You must log in to view your tax record! :O ")
                continue

            print(f"{'IC Number':<15} {'Income':>12} {'Relief':>12} {'Tax':>12}")
            print("_" * 55)
            print(f"{current_user['IC Number']:<15} "
                    f"RM{float(current_user['Income']):>10,.2f} "
                    f"RM{float(current_user['Tax Relief']):>10,.2f} "
                    f"RM{float(current_user['Tax Payable']):>10,.2f}")
